    ...
    if (!email || !password) {
      ...
      if (result.status == 200) {
        if (result.data.length > 0) {
          ...
          toast.error("Login failed!! inavalid email or password!!")
        }
      }
      else {
  ...
  <>
    <div className="container-fluid d-flex justify-content-center align-items-center" style={{ height: '80vh' }}>
      <div className="w-50 border shadow bg-light border-3 border-dark p-3">
        <Row>
          <Col>
            <img src="https://img.freepik.com/free-vector/login-concept-illustration_114360-739.jpg" className='w-100 h-100' alt="logo-img" />
          </Col>
          <Col>
            {
              authStatus ?
